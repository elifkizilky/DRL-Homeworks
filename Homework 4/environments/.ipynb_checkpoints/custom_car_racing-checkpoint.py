'''
    Taken from: https://github.com/TychoTheTaco/Car-Racing
'''
    ...
    def step(self, action):
        ...
        for i in range(self._step_repeat):
            ...
            if np.mean(observation[64:80, 42:54, 1]) > 120:
                reward -= 0.25
                #print(f'going off the track deducted reward: {reward}')

            # Punish brake usage
            if action is not None:
                punish = np.interp(action[2], [0, 1], [0, 0.05])
                #print(f'reward: {reward} | Punish: {punish}')
                reward -= punish

            # End early if the agent consistently does poorly
            self._reward_history.append(reward)
            if np.mean(self._reward_history) < -0.1: #-0.1
                #print(np.mean(self._reward_history))
                #print("done poorly")
                done = True
